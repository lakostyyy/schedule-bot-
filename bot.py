...
DAY_NAMES = {0:"ПОНЕДІЛОК",1:"ВІВТОРОК",2:"СЕРЕДА",3:"ЧЕТВЕР",4:"П'ЯТНИЦЯ"}

FALLBACK_SCHEDULE = {
    "ПОНЕДІЛОК":{1:"Література",2:"Математика",3:"Історія України",4:"Англійська мова",5:"Хімія",6:"Англ 1",7:"Фізкультура/Шахи",8:"Класні теревені"},
    "ВІВТОРОК":{1:"Фізика",2:"Література",3:"Математика",4:"Математика",5:"Англійська мова",6:"Українська мова",7:"Англ 2",8:"Нім мова В1"},
    "СЕРЕДА":{1:"Проєкт менедж",2:"Проф орієнт",3:"Інформатика",4:"Інформатика",5:"Англійська мова",6:"Фізика",7:"Біологія",8:"Українська мова"},
    "ЧЕТВЕР":{1:"Українська мова",2:"Англійська мова",3:"Всесвітня історія",4:"Всесвітня історія",5:"Фізкультура",6:"Географія",7:"Географія",8:"Нім мова В1"},
    "П'ЯТНИЦЯ":{1:"Емпатика",2:"Англійська мова",3:"Українська мова",4:"Англ 1",5:"Хімія",6:"Математика",7:"Шахи",8:"—"},
}
...
def parse_csv(content):
    ...
    current_day = None
    class_col = None
    day_keywords = {"ПОНЕДІЛОК":"ПОНЕДІЛОК","ВІВТОРОК":"ВІВТОРОК","СЕРЕДА":"СЕРЕДА","ЧЕТВЕР":"ЧЕТВЕР","П'ЯТНИЦЯ":"П'ЯТНИЦЯ","ПЯТНИЦЯ":"П'ЯТНИЦЯ"}
...
def get_schedule():
    ...
    _cache = parse_csv(csv_content) if csv_content else FALLBACK_SCHEDULE
    _cache_date = today
    return _cache
...
def get_status(schedule):
    ...
    for i, (s, e) in enumerate(LESSON_TIMES):
        if t2m(s) <= nm < t2m(e):
            return {"type":"lesson","num":i+1,"subject":ds.get(i+1,"—"),"remaining":t2m(e)-nm,"next":ds.get(i+2),"break":BREAK_TIMES[i] if i < len(BREAK_TIMES) else None}
    for i, (s, e, l, lb) in enumerate(BREAK_TIMES):
        if t2m(s) <= nm < t2m(e):
            return {"type":"break","label":lb,"remaining":t2m(e)-nm,"next_num":i+2,"next":ds.get(i+2,"—")}
...
def fmt_status(s):
    t = s["type"]
    if t == "weekend": return "🎉 Сьогодні вихідний!"
    if t == "lesson":
...
async def cmd_start(u: Update, c):
    await u.message.reply_text("👋 Привіт! Бот розкладу *10А*\n\n/now — що зараз\n/today — сьогодні\n/tomorrow — завтра\n/week — весь тиждень\n/mon /tue /wed /thu /fri — день", parse_mode="Markdown")
...
async def cmd_today(u: Update, c):
    ...
    if wd >= 5: await u.message.reply_text("🎉 Вихідний!"); return
    day = DAY_NAMES[wd]
...
async def cmd_tomorrow(u: Update, c):
    ...
    if wd >= 5: await u.message.reply_text("🎉 Завтра вихідний!"); return
    day = DAY_NAMES[wd]
    await u.message.reply_text(fmt_day(day, get_schedule().get(day,{})), parse_mode="Markdown")
